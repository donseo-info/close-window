/* ═══════════════════════════════════════════════════════
   POPUP C — Эффект Зейгарник (прогресс + 1 шаг)
   Дизайн: белый, синий акцент, прогресс-бар, бизнес
═══════════════════════════════════════════════════════ */
(function () {
  "use strict";
  ...
  function injectStyles() {
    ...
    s.textContent = [
      ...
      "#pc-box{",
        ...
        "box-shadow:0 12px 50px rgba(10,20,80,.18);",
      "}",
      ...
      "}",
      ".pc-head-label{",
        "font-family:system-ui,'Segoe UI',sans-serif;font-size:12px;font-weight:500;",
      ...
      ".pc-track{",
        "height:7px;background:rgba(255,255,255,.2);",
      ...
      "}",
      ".pc-chk.done{background:#2563eb;color:#fff;}",
      ".pc-chk.pend{background:#f0f0f0;color:#ccc;font-size:11px;}",

      /* форма */
      ".pc-label{",
        "font-family:system-ui,'Segoe UI',sans-serif;font-size:13px;font-weight:500;",
        "color:#333;display:block;margin-bottom:6px;",
      "}",
      ".pc-phone{",
        "width:100%;box-sizing:border-box;",
        "border:1.5px solid #dde2ee;border-radius:7px;",
        "padding:11px 14px;",
        "font-family:system-ui,'Segoe UI',sans-serif;font-size:15px;color:#111;",
        "outline:none;transition:border-color .2s;background:#f9fbff;",
      "}",
      ".pc-phone::placeholder{color:#bbb;}",
      ".pc-phone:focus{border-color:#2563eb;background:#fff;}",
      ...
      "}",
      ".pc-btn:hover{background:#1d4ed8;transform:translateY(-1px);}",
      ...
      ".pc-ok h3{font-family:system-ui,'Segoe UI',sans-serif;font-size:21px;font-weight:700;color:#2563eb;margin:0 0 6px;}",
  ...
  function buildDOM() {
    ...
    ov.innerHTML = [
      '<div id="pc-box">',
        '<div class="pc-head">',
          ...
          '<span class="pc-head-label">🤔 Подождите секунду</span>',
          '<h2>Уже почти всё готово…<br>Остался всего 1 шаг!</h2>',
          '<div class="pc-progress-row">',
            '<span>Ваш прогресс</span>',
            '<span class="pc-pct">80%</span>',
          '</div>',
          '<div class="pc-track"><div class="pc-fill" id="pc-fill"></div></div>',
          '<div class="pc-steps-row">',
            '<div class="pc-step done"></div>',
            '<div class="pc-step done"></div>',
            '<div class="pc-step done"></div>',
            '<div class="pc-step done"></div>',
        ...
        '<div class="pc-body">',
          '<ul class="pc-list">',
            '<li class="done"><span class="pc-chk done">✓</span>Цели определены</li>',
            '<li class="done"><span class="pc-chk done">✓</span>Тарифный план выбран</li>',
            '<li class="done"><span class="pc-chk done">✓</span>Бюджет согласован</li>',
            '<li class="done"><span class="pc-chk done">✓</span>Условия изучены</li>',
            '<li><span class="pc-chk pend">○</span>Контакт для связи с менеджером</li>',
          '</ul>',
          '<div id="pc-form-area">',
            '<form id="pc-form" novalidate>',
              ...
              '<div class="pc-messengers">',
                '<button type="button" class="pc-m tg" data-m="tg">' + ICONS.tg + 'Telegram</button>',
                '<button type="button" class="pc-m wa" data-m="wa">' + ICONS.wa + 'WhatsApp</button>',
                '<button type="button" class="pc-m mx" data-m="mx">' + ICONS.mx + 'Max</button>',
              '</div>',
              '<button class="pc-btn" type="submit"><span>✔</span> Завершить оформление</button>',
            '</form>',
    ...
    var activeM = null;
    ov.querySelectorAll(".pc-m").forEach(function(btn){
      btn.addEventListener("click", function(){
        ov.querySelectorAll(".pc-m").forEach(function(b){ b.classList.remove("sel"); });
    ...
    document.getElementById("pc-form").addEventListener("submit", function(e){
      e.preventDefault();
      var tel = document.getElementById("pc-tel").value.trim();
      if (!tel) { document.getElementById("pc-tel").focus(); return; }
      if (_onSubmitCb) _onSubmitCb({ variant:"C", phone: tel, messenger: activeM });
      document.getElementById("pc-form-area").innerHTML = [
        '<div class="pc-ok">',
          '<span class="pc-ok-ico">🚀</span>',
          '<h3>Отлично! Вы на финише.</h3>',
  ...
  function show() {
    injectStyles();
    if (!document.getElementById("pc-overlay")) buildDOM();
    var el = document.getElementById("pc-overlay");
    el.style.display = "flex";
    requestAnimationFrame(function(){
      requestAnimationFrame(function(){
        el.classList.add("pc-on");
        setTimeout(function(){
          var f = document.getElementById("pc-fill");
          if (f) f.style.width = "80%";
        }, 250);
      });
    });
  }
  ...
  function hide() {
    ...
    if (_onCloseCb) _onCloseCb({ variant:"C" });
