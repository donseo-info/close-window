/* ═══════════════════════════════════════════════════════
   POPUP A — Срочная скидка + FOMO
   Дизайн: белый, красный акцент, живой таймер
═══════════════════════════════════════════════════════ */
(function () {
  "use strict";
  ...
  function injectStyles() {
    ...
    s.textContent = [
      ...
      ".pa-phone:focus{border-color:#e02020;}",
      ...
      "}",
      ".pa-submit:hover{background:#c01818;transform:translateY(-1px);}",
      ...
      ".pa-close{",
        ...
        "transition:background .15s;z-index:10;flex-shrink:0;",
      "}",
  ...
  function startTimer() {
    var total = 3 * 60;
    function tick() {
      var m = document.getElementById("pa-min");
      var s = document.getElementById("pa-sec");
      if (!m) { clearInterval(_timerInterval); return; }
      m.textContent = String(Math.floor(total / 60)).padStart(2,"0");
      s.textContent = String(total % 60).padStart(2,"0");
      if (total-- <= 0) clearInterval(_timerInterval);
    }
    tick();
  ...
  function buildDOM() {
    ...
    ov.innerHTML = [
      '<div id="pa-box">',
        '<div class="pa-head">',
          ...
          '<div class="pa-badge">🔥 Только сегодня</div>',
          '<h2>Уже уходите?<br>Подождите — скидка&nbsp;−10%!</h2>',
          '<p>Оставьте номер и получите промокод прямо сейчас</p>',
          '<div class="pa-timer">',
          ...
          '<div id="pa-form-area">',
            ...
            '<form id="pa-form" novalidate>',
              ...
              '<div class="pa-messengers">',
                '<button type="button" class="pa-m-btn tg" data-m="tg">' + ICONS.tg + 'Telegram</button>',
                '<button type="button" class="pa-m-btn wa" data-m="wa">' + ICONS.wa + 'WhatsApp</button>',
                '<button type="button" class="pa-m-btn mx" data-m="mx">' + ICONS.mx + 'Max</button>',
              '</div>',
              '<button class="pa-submit" type="submit">Получить скидку −10%</button>',
            '</form>',
    ...
    var activeM = null;
    ov.querySelectorAll(".pa-m-btn").forEach(function(btn){
      btn.addEventListener("click", function(){
        ov.querySelectorAll(".pa-m-btn").forEach(function(b){ b.classList.remove("active"); });
    ...
    document.getElementById("pa-form").addEventListener("submit", function(e){
      e.preventDefault();
      var tel = document.getElementById("pa-tel").value.trim();
      if (!tel) { document.getElementById("pa-tel").focus(); return; }
      if (_onSubmitCb) _onSubmitCb({ variant:"A", phone: tel, messenger: activeM });
      document.getElementById("pa-form-area").innerHTML = [
        '<div class="pa-success">',
          '<span class="pa-success-ico">🎉</span>',
          '<h3>Промокод отправлен!</h3>',
          '<p>Менеджер напишет вам в течение 5 минут.</p>',
        '</div>',
      ].join("");
  ...
  function show() {
    injectStyles();
    if (!document.getElementById("pa-overlay")) buildDOM();
    var el = document.getElementById("pa-overlay");
    el.style.display = "flex";
    requestAnimationFrame(function(){
      requestAnimationFrame(function(){ el.classList.add("pa-on"); });
    });
    startTimer();
  }
  ...
  function hide() {
    var el = document.getElementById("pa-overlay");
    if (!el) return;
    el.classList.remove("pa-on");
    clearInterval(_timerInterval);
    if (_onCloseCb) _onCloseCb({ variant:"A" });
